...
class Midtrans_Snapio_PaymentController
    extends Mage_Core_Controller_Front_Action
{
    ...
    public function redirectAction()
    {
        ...
        Config::$isProduction = Mage::getStoreConfig('payment/snapio/environment') == 'production' ? true : false;
        Mage::log('environment' . Mage::getStoreConfig('payment/snapio/environment'), null, 'snap.log', true);
        Config::$serverKey = Mage::getStoreConfig('payment/snapio/server_key');
        Mage::log('server key' . Mage::getStoreConfig('payment/snapio/server_key'), null, 'snap.log', true);
        Config::$is3ds = true;
        Config::$isSanitized = true;

        $enable_snap_redirect = Mage::getStoreConfig('payment/snapio/snapredirect') == '1' ? true : false;

        $oneClick = Mage::getStoreConfig('payment/snapio/oneclick') == '1' ? true : false;

        $terms_offline = explode(',', Mage::getStoreConfig('payment/snapio/installmentterm'));
        ...
        $bank = Mage::getStoreConfig('payment/snapio/bank');
        ...
        $terms = array();
        foreach ($terms_offline as $terms_offline) {
            $terms[] = (int)$terms_offline;
        };

        $installment_term['offline'] = $terms;
        ...
        if ($totalPrice >= Mage::getStoreConfig('payment/snapio/threshold')) {
        ...
        if ($enable_snap_redirect == false) {
            ...
            try {
                ...
                Mage::log('debug:' . print_r($payloads, true), null, 'snap.log', true);
                Mage::log(json_encode($payloads), null, 'snap.log', true);
                Mage::log('SNAP Rederict URL = ' . $redirectUrl, null, 'snap.log', true);
                $this->_getCheckout()->setToken($redirectUrl);
                $this->_getCheckout()->setEnv(Mage::getStoreConfig('payment/snapinst/environment'));
