...
class Midtrans_Snapinstmigs_PaymentController
  ...
  public function redirectAction() {
    ...
    Veritrans_Config::$isProduction =
        Mage::getStoreConfig('payment/snapinstmigs/environment') == 'production'
        ? true : false;

    Veritrans_Config::$serverKey =
        Mage::getStoreConfig('payment/snapinstmigs/server_key');
    ...
    if ($current_currency != 'IDR') {
      $conversion_func = function ($non_idr_price) {
          return $non_idr_price *
              Mage::getStoreConfig('payment/snapinstmigs/conversion_rate');
        };
    ...
    $installment_term['bca'] = array(6,12);
    //$installment_term['bri'] = array(3,6,9,12,18,24,36);
    //$installment_term['maybank'] = array(3,6,9,12,18,24,36);
    ...
    if ($totalPrice >= Mage::getStoreConfig('payment/snapinstmigs/threshold') ) {
    ...
    try {
      ...
      Mage::app()->getFrontController()->getResponse()->setRedirect(Mage::getBaseUrl().'snapinstmigs/payment/opensnap');
  ...
  public function opensnapAction(){
      ...
      $template = 'snapinstmigs/open.phtml';
