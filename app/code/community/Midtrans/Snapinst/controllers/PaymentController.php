...
class Midtrans_Snapinst_PaymentController extends Mage_Core_Controller_Front_Action
{
    ...
    public function redirectAction()
    {
        ...
        Config::$isProduction = Mage::getStoreConfig('payment/snapinst/environment') == 'production' ? true : false;

        Config::$serverKey = Mage::getStoreConfig('payment/snapinst/server_key');
        //Mage::log('server key'.Mage::getStoreConfig('payment/snap/server_key'),null,'snap.log',true);
        Config::$is3ds = true;
        Config::$isSanitized = true;

        $enable_snap_redirect = Mage::getStoreConfig('payment/snapinst/snapredirect') == '1' ? true : false;
        $oneClick = Mage::getStoreConfig('payment/snapinst/oneclick') == '1' ? true : false;
        ...
        if ($current_currency != 'IDR') {
            $conversion_func = function ($non_idr_price) {
                return $non_idr_price *
                    Mage::getStoreConfig('payment/snapinst/conversion_rate');
            };
        ...
        $bin = Mage::getStoreConfig('payment/snapinst/bin');
        $bank = Mage::getStoreConfig('payment/snapinst/bank');
        $payloads = array();
        ...
        if ($totalPrice >= Mage::getStoreConfig('payment/snapinst/threshold')) {
    ...
    // Midtrans will send notification of the payment status, this is only way we
    ...
    public function notificationAction()
    {
        ...
        Config::$serverKey = Mage::getStoreConfig('payment/snap/server_key_v2');
